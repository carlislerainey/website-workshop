...
I built this guide with a lot of help from Claude Code. I walked Claude Code through how I recommend using Quarto to build an academic website, and we built a simple website together. Then Claude Code helped me draft the step-by-step instructions, then rewrite and verify them.
:::
...
![The home page you are about to build.](screenshots/home-jolla.png){fig-alt="A centered academic home page: a round headshot, the name Your Name, a short bio, and a row of buttons labeled Email, CV, Google Scholar, and ORCID."}

## Examples

Have a look at the sites below. These are made using a process similar to the one described in this guide.
...
![The Build tab's Render Website button.](screenshots/rstudio-render-button.png){fig-alt="RStudio's Build tab, top right, with the Render Website button."}
...
![The site Quarto starts you with.](screenshots/skeleton-home.png){fig-alt="The default Quarto website: a plain white page titled USERNAME.github.io with Home and About links in the navbar."}
...
```yaml
# The whole site is configured in this one file. The pages are the .qmd files
# next to it; the rendered site goes in docs/.
# Everything this file accepts: https://quarto.org/docs/reference/projects/websites.html

project:
  type: website
  output-dir: docs        # GitHub Pages can serve a site straight from docs/.

website:
  title: "Your Name"      # Shown at the left end of the navbar and in the browser tab.
  navbar:
    left:
      - text: Home
        href: index.qmd
      - research.qmd      # Link text comes from the page's own title.
      - teaching.qmd
      - text: CV
        href: cv.pdf      # The CV is a PDF, not a page. Replace the file; keep the name.
  # Three additions you will probably want, in the order people usually want them:
  #   - Another page (talks, software, data). Create talks.qmd with a title: line at
  #     the top, then add "- talks.qmd" to the navbar list above. A blog is one step
  #     more, a page that lists posts: https://quarto.org/docs/websites/website-blog.html
  #   - Link previews. When someone shares your site on Bluesky or in Slack, show a
  #     card with your name and photo instead of a bare URL: add
  #     "site-url: https://USERNAME.github.io" and "open-graph: true" at this indent.
  #     (site-url also gives search engines a sitemap.)
  #   - Visitor counts, if you want them: create a free Google Analytics property,
  #     then add its ID here as google-analytics: "G-XXXXXXXXXX", at this indent.
  # More additions like these: https://quarto.org/docs/websites/website-tools.html

format:
  html:
    theme: cosmo          # Quarto's default look. Try flatly (dark navbar), litera (serif
                          # text), or sandstone. Change the word, re-render. All 25 themes,
                          # with pictures: https://quarto.org/docs/output-formats/html-themes.html
    css: styles.css       # Extra styling on top of the theme. The file is empty; your own
                          # CSS rules go there if a theme setting ever isn't enough.
```
...
![The research page with the sample content.](screenshots/research-page.png){fig-alt="The rendered research page: a navbar across the top, then headings for Peer-Reviewed Articles, Book Chapters, Working Papers, and Early Projects, each with sample citations."}
...
## Check your site before you publish
...
::: {layout-ncol=2}
![trestles: photo on the left.](screenshots/home-trestles.png){fig-alt="The home page in the trestles layout: photo and buttons in a left column, bio on the right."}

![solana: photo on the right.](screenshots/home-solana.png){fig-alt="The home page in the solana layout: bio on the left, photo on the right."}
:::
...
::: {layout-ncol=2}
![flatly: dark navbar.](screenshots/theme-flatly.png){fig-alt="The home page in the flatly theme, with a dark navy navbar."}

![sandstone: warm and muted.](screenshots/theme-sandstone.png){fig-alt="The home page in the sandstone theme, with warm gray tones."}

![litera: serif body text.](screenshots/theme-litera.png){fig-alt="The home page in the litera theme, with serif body text."}

![zephyr: rounded and airy.](screenshots/theme-zephyr.png){fig-alt="The home page in the zephyr theme, with rounded buttons and generous spacing."}
:::

3. Just for fun, try `sketchy` and/or `vapor`. (Then change back to your favorite.)

::: {layout-ncol=2}
![sketchy.](screenshots/theme-sketchy.png){fig-alt="The home page in the sketchy theme, drawn in a hand-sketched comic style."}

![vapor.](screenshots/theme-vapor.png){fig-alt="The home page in the vapor theme, in neon purple vaporwave colors."}
:::

## Publish your site with GitHub Desktop
...
![The Changes tab before the first commit.](screenshots/github-desktop-changes.png){fig-alt="GitHub Desktop's Changes tab listing many files, with the Summary box and the Commit to main button at the bottom left."}

::: {.callout-important}
Uncheck **Keep this code private** before you publish. With a free account, GitHub Pages only works for *public* repositories.
:::

![The Publish dialog, with the private box unchecked.](screenshots/github-desktop-publish.png){fig-alt="GitHub Desktop's Publish repository dialog, with the name filled in and Keep this code private unchecked."}
...
![The Pages settings, pointed at the docs folder.](screenshots/pages-settings.png){fig-alt="GitHub's Pages settings: Source set to Deploy from a branch, with the branch dropdown and the folder set to /docs."}
...
## Extension: your own web address
...
1. On Namecheap, click **Domain List → Manage → Advanced DNS**. 
   ...
   b. Add nine records. [Namecheap's own GitHub Pages guide](https://www.namecheap.com/support/knowledgebase/article.aspx/9645/2208/how-do-i-link-my-domain-to-github-pages/) describes the first five. The four `AAAA` records point the bare domain at GitHub over IPv6.
